...
router.post("/register", async (req, res) => {
  ...
  ).run(id, email.toLowerCase().trim(), hash, name.trim(), "admin");

  const payload = { sub: id, email: email.toLowerCase().trim(), name: name.trim(), role: "admin" };
  const token = jwt.sign(payload, JWT_SECRET, { expiresIn: "7d" });
  res.cookie("auth_token", token, COOKIE_OPTS);
...
router.post("/invite", requireRole("admin", "founder"), async (req, res) => {
  const { email, role = "readonly" } = req.body;
  if (!email) return res.status(400).json({ error: "Email required" });
  if (!["admin", "founder", "seo", "readonly"].includes(role))
...
router.get("/users", requireRole("admin", "founder"), (req, res) => {
  const db = getDb();
...
router.put("/users/:id/role", requireRole("admin", "founder"), (req, res) => {
  const { role } = req.body;
  if (!["admin", "founder", "seo", "readonly"].includes(role))
    return res.status(400).json({ error: "Invalid role" });
...
router.delete("/users/:id", requireRole("admin", "founder"), (req, res) => {
  if (req.params.id === req.user.sub)

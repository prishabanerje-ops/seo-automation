...
export default function CrawlRunner() {
  ...
  const [sfViewHistory, setSfViewHistory] = useState({});  // jobId → boolean (for history cards)
  ...
  function loadRecentCrawls() {
    api.get("/crawl/history").then(r => {
      setRecentCrawls(r.data.filter(j => j.status !== "running").slice(0, 10));
    }).catch(() => {});
  }

  useEffect(() => { loadRecentCrawls(); }, []);
  ...
  function subscribeToJob(jobId) {
    ...
    socket.on(`crawl:complete:${jobId}`, ({ status }) => {
      ...
      setTimeout(loadRecentCrawls, 1500); // refresh history after job lands in DB
    });
  }
